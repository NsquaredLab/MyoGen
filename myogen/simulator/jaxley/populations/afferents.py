"""
Afferent neuron populations for proprioceptive feedback - Jaxley Backend.
...
class AffIa__Pool(_Pool):
    """
    Container for a population of afferent Ia neurons - Jaxley Backend.
...
class AffII__Pool(_Pool):
    """
    Container for a population of afferent II neurons - Jaxley Backend.
...
class AffIb__Pool(_Pool):
    """
    Container for a population of afferent Ib neurons - Jaxley Backend.

    Manages a collection of AffIb (type Ib afferent) cells that provide
    feedback from Golgi tendon organs to spinal circuits.
    ...
    def __init__(
        ...
        for i, (rt_i, vcon_i) in enumerate(zip(rt, vcon)):
            ...
            ib.create_axon(length__m=axon_length__m, conduction_velocity__m_per_s=vcon_i)
            _cells.append(ib)
