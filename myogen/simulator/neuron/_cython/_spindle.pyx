...
cdef class _Spindle__Cython():
    ...
    def __init__(self, double tstop, double dt, dict spinD):
        ...
        self.tau2 = spinD["decay_time_constant__ms"]
        self.tInt = 0
        self.dt = dt * 1e-3  # [s]
    ...
    cdef double dfdtBag1(self, double t, double aBag1, double gDyn) except? -1:
        cdef double t1
        t1 = gDyn ** self.P / (gDyn ** self.P + self.fBag1 ** self.P)
        return (t1 - aBag1) / self.tau1

    cdef double dfdtBag2(self, double t, double aBag2, double gStat) except? -1:
        cdef double t1
    ...
    cdef double ddTen(self, double t, double T, double dT, double L, double V, double A, Py_ssize_t ifType) nogil:
    ...
    cdef void runge(self, double L, double V, double A, double gDyn,
                    double gStat) except*:
        cdef Py_ssize_t i
        self.aBag1[self.tInt + 1] = self.rk4Bag1(
            self.time[self.tInt],
            self.aBag1[self.tInt],
            gDyn
        )
        self.aBag2[self.tInt + 1] = self.rk4Bag2(
            self.time[self.tInt],
    ...
    cdef (double, double) rk4z(
        ...
        self.k2z[i] = self.ddTen(
            t + self.dt / 2, y + self.dt * self.k1y[i] / 2,
            z + self.dt * self.k1z[i] / 2,
            L,
            V,
            A,
        ...
        self.k3z[i] = self.ddTen(
            t + self.dt / 2, y + self.dt * self.k2y[i] / 2,
            z + self.dt * self.k2z[i] / 2,
            L,
            V,
            A,
        ...
        self.k4z[i] = self.ddTen(
            t + self.dt, y + self.dt * self.k3y[i],
            z + self.dt * self.k3z[i],
            L,
            V,
            A,
