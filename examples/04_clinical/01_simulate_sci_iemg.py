...
for ax, label in zip(axes, labels):
    ...
    ax.plot(drive.times.rescale(pq.s).magnitude, drive.magnitude, linewidth=0.3)
...
for ax, label in zip(axes, labels):
    ...
    ax_rate = ax.twinx()
    ax_rate.plot(rate_t__s, rate__pps, color="black", linewidth=0.375, alpha=0.85)
...
for ax, label in zip(axes, labels):
    ...
    ax.plot(iemg.times.rescale(pq.s).magnitude, iemg.magnitude, linewidth=0.15)
